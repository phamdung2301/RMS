...
### Bước 4: Cấu hình dịch vụ bên thứ ba (Google OAuth2, Gmail SMTP, Gemini API)
Để đảm bảo an toàn thông tin và tuân thủ chính sách bảo mật mã nguồn (tránh lộ lọt credentials trên GitHub), toàn bộ thông tin nhạy cảm đã được chuyển đổi sang dạng cấu hình linh hoạt thông qua **biến môi trường (Environment Variables)** với các placeholder mặc định trong [application.properties](file:///d:/swp/src/main/resources/application.properties).

Lập trình viên có thể cấu hình các thông số này bằng 2 cách:
* **Cách 1 (Khuyên dùng - Bảo mật nhất)**: Cấu hình biến môi trường trực tiếp trong cấu hình chạy của IDE (IntelliJ IDEA Run/Debug Configurations > Environment variables):
  - `GOOGLE_OAUTH_CLIENT_ID`: Client ID của tài khoản Google OAuth2.
  - `GOOGLE_OAUTH_CLIENT_SECRET`: Client Secret của Google OAuth2.
  - `GEMINI_API_KEY`: API Key của mô hình Google Gemini.
  - `SPRING_MAIL_USERNAME`: Địa chỉ Gmail dùng để gửi thư hệ thống.
  - `SPRING_MAIL_PASSWORD`: Mật khẩu ứng dụng (App Password) của tài khoản Gmail đó.
* **Cách 2**: Chỉnh sửa trực tiếp các giá trị mặc định sau dấu `:` trong file `application.properties` ở máy cá nhân.
  > [!IMPORTANT]
  > Nếu chọn Cách 2, **tuyệt đối không commit file cấu hình chứa key thật lên Git**. Bạn nên chạy lệnh sau trong terminal để báo Git bỏ qua việc theo dõi thay đổi cục bộ đối với file cấu hình:
  > ```bash
  > git update-index --assume-unchanged src/main/resources/application.properties
  > ```
  > *(Khi muốn theo dõi lại file, chạy lệnh: `git update-index --no-assume-unchanged src/main/resources/application.properties`)*

> [!NOTE]
> Để nhận file chứa thông tin tài khoản phát triển (Dev Credentials) dùng chung của nhóm (Google Client ID, Gemini Key và Gmail App Password mẫu), vui lòng liên hệ trực tiếp với Tech Lead qua kênh trao đổi nội bộ của đội ngũ (Slack/Discord/Teams).
