...
def build_full_db_design():
    md_content = """# THIẾT KẾ CƠ SỞ DỮ LIỆU TOÀN DIỆN (COMPREHENSIVE DATABASE DESIGN)
...
"""
    ...
    md_path = os.path.join(doc_dir, "Database_Design.md")
    
    # 1. Write the new comprehensive markdown file
    print(f"Writing full Database_Design.md to {md_path}...")
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(md_content)
    print("Markdown file successfully updated!")
    
    # 2. Run the compiler to compile Database_Design.md to Database_Design.docx
    print("Compiling Database_Design.md to Database_Design.docx using compile_docs.py...")
    try:
        # Run python d:/swp/compile_docs.py via subprocess to compile
        subprocess.run(["python", "d:/swp/compile_docs.py"], check=True)
        print("Success! Database_Design.docx has been completely regenerated with 43 tables!")
    except Exception as e:
        print(f"Error during compilation: {e}")

if __name__ == "__main__":
    build_full_db_design()
